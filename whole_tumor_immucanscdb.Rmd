...
met2 <- annotate_cells(object = seu[1:2],
...
rib <- SignatuR::GetSignature(SignatuR$Hs$Compartments$Ribo)

all.g <- all.genes[rib,]
```

```{r}
avg.test <- CellAvgExpression(object,
                              group.by = "annotation",
                              gene.filter = list(c("IL10", "BLIMP1", "BCL6")),
                              GO_accession = "GOHAHA"
                              )
```


# Hit test w/o proportions

```{r}
add.sig <- GetSignature(SignatuR$Hs$Programs)


hit.dir <- annotate_cells(seu$Lambrechts_2018_29988129,
                           scGate.model = models.TME,
                           ref.maps = ref.maps,
                           return.Seurat = F,
...
```{r}
seu.list <- SplitObject(seu$Lambrechts_2018_29988129,
                        split.by = "group")

hit.dir <- annotate_cells(seu.list$`6`,
                           scGate.model = models.TME,
                           ref.maps = ref.maps,
                           return.Seurat = F,
                           additional.signatures = add.sig,
...
```{r}
hit.dir <- annotate_cells(seu.list$`1`,
                           scGate.model = models.TME,
                           ref.maps = ref.maps,
                           return.Seurat = T,
                           additional.signatures = add.sig,
...
```{r}
hit.list <- annotate_cells(seu.list[1:2],
                           remerge = F,
                           # scGate.model = models.TME,
                           ref.maps = ref.maps,
                           return.Seurat = T,
                           additional.signatures = add.sig,
                           group.by.avg.expr = c("annotation",
                                                 "scGate_multi",
                                                 "Consensus_subtypes"),
                           ncores = 9)
...
```{r}
gr <- c("annotation","scGate_multi","Consensus_subtypes")
add.sig <- GetSignature(SignatuR$Hs$Programs)
...
hit.list <- annotate_cells(seu.list[1:2],
                           ...
                           return.Seurat = T,
                           additional.signatures = add.sig,
                           group.by.avg.expr = gr,
                           annot.cols.freq = gr,
                           ncores = 9)

tt <- CreateHiTObject(hit.list[[2]],
                      ref.maps = ref.maps,
                       scGate.model = models.TME,
                       group.by = gr,
                      annot.cols.freq = gr,
