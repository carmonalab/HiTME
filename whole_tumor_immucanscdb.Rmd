...
scGate.links <- list("CD8" = "CD8T",
                  "CD4" = "CD4T",
                  "DC" = "panDC",
                  "MoMac" = c("Monocyte", "Macrophage")
                  )
for(a in names(ref.maps)){
  ref.maps[[a]]@misc$scGate_link <- scGate.links[[a]]
}
```
...
for(a in names(seu)){
  ...
  for(b in names(dataset.list)){
    ...
    hit.list[[a]] <- get.HiTObject(seu.list[[b]],
                          ref.maps = ref.maps,
                           scGate.model = models.TME,
                           group.by = gr,
                          annot.cols.freq = gr,
    ...
    error = function(e){
      ...
      print(unique(seu.list[[b]]$annotation))
    })
    } else {
...
## Test new version
```{r}
test <- Run.HiTME(seu$Lambrechts_2018_29988129[,1:500],
                  ref.maps = ref.maps,
                  return.Seurat = F,
                  ncores = 9)
```
...
```{r}
BPPARAM <- BiocParallel::MulticoreParam(workers = 9, progressbar = T)
ptest <- ProjecTILs.classifier.multi(object,
                                     ref.maps = ref.maps,
                                     BPPARAM = BPPARAM)
```

```{r}
object <- scGate(seu$Peng_2019_31273297, model = models.TME, ncores = 9, verbose = T)
```
